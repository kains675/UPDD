...
[![Release](https://img.shields.io/badge/release-v0.7.1-blue.svg)](https://github.com/kains675/UPDD/releases/tag/v0.7.1-paper1-v2)
[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.20067323.svg)](https://doi.org/10.5281/zenodo.20067323)

## Overview

UPDD is an end-to-end *in silico* peptide drug design system that couples generative protein design with physics-based binding evaluation. The pipeline generates, evaluates, and ranks cyclic peptide candidates targeting user-specified protein interfaces — from backbone generation to binding free energy estimation.

This release (v0.7.1) accompanies **Paper 1** — a Capability Level 1 methodology release documenting the σ_btwn / σ_w ensemble-quality decomposition, the Convergence Index dual interpretation, the four-layer onion-peeling verification protocol, and the simulated active-learning cycle on the six-target ncAA benchmark. See the [Citation](#citation) section below for the ChemRxiv preprint reference. (Development continues beyond v0.7.1 — e.g. Track B alchemical RBFE infrastructure, validation in progress.)
...
| ncAA Integration | Custom mutation engine | Non-canonical amino acid substitution + RESP (AM1-BCC fallback) / GAFF2 |
| Molecular Dynamics | OpenMM (AMBER ff14SB) | Restrained MD with **3-pass NaN recovery** |
| Electronic Structure | gpu4pyscf (ωB97X-D/6-31G*) | QM/MM single-point interaction energy (ranking-only ΔΔ_int) |
| Binding Energy (endpoint) | OpenMM (PBSA / GBn2) | MM-PBSA / MM-GBSA ΔG estimation |
| Binding Energy (alchemical) | OpenMM `ATMForce` + ATS + UWHAM | Relative binding FE (Track B) — *validation in progress* |

## Why UPDD?

Most AI-driven peptide design tools stop at structure prediction or ncAA placement, and the widely used backbone/sequence models (RFdiffusion, ProteinMPNN) were trained on canonical amino acids and **cannot natively model ncAAs** [[1]](#references). UPDD's distinguishing aim is to put *de novo* backbone generation, binder design, ncAA substitution, and physics-based binding evaluation in a single orchestrated pipeline. The table credits each tool's actual strengths:

| Capability | RFdiffusion | NCFlow | CyclicChamp | **UPDD** |
|------------|:---:|:---:|:---:|:---:|
| *De novo* backbone generation | ✅ | — | ✅ | ✅ |
| Protein binder design | ✅ | — | — | ✅ |
| ncAA placement / substitution | — | ✅ | ✅ | ✅ |
| Alchemical RBFE | — | ✅ | — | ⚠️ in progress |
| ML affinity prefilter | — | ✅ | — | — (roadmap) |
| QM/MM (DFT) | — | — | — | ✅ |
| MM-PBSA / MM-GBSA ΔG | — | — | — | ✅ |
| Single orchestrated pipeline | — | — | — | ✅ |

Legend: ✅ present/demonstrated · ⚠️ in progress · — not provided.
...
- **Level 2 — Cross-system reproducibility** (v0.8 target): sign-significance reproduces on three+ systems spanning multiple ncAA classes; bit-identical WT control as routine reproducibility benchmark; Track B alchemical RBFE validated as a second estimator.
...
The v0.7.1 release described in Paper 1 is explicitly a Level 1 release.

## Key Features

- **ncAA Support**: 25 registered ncAAs at v0.7 (registry-based; MTR-class verified, N-methyl class deferred to N-methyl overlay)
...
- **Alchemical RBFE (Track B)**: OpenMM `ATMForce` soft-core + ATS two-copy + UWHAM, async replica-exchange λ-ladder — infrastructure under validation (not yet a validated binding predictor)
...
- `tests/` — regression suite (1145 pass / 27 skip / 1 pre-existing fail)
...
1. Lee, J. S. & Kim, P. M. "Design of peptides with non-canonical amino acids using flow matching" (NCFlow). *bioRxiv* 2025.07.31.667780 (2025). — Fixed-backbone ncAA placement + ATM/AEV-PLIG variant ranking; documents that RFdiffusion / BindCraft cannot model ncAAs.
...
6. Valsson, I., Warren, M. T., Magarkar, A., Biggin, P. C. & Deane, C. M. "Narrowing the gap between machine learning scoring functions and free energy perturbation using augmented data" (AEV-PLIG). *Commun. Chem.* **8**, 41 (2025).
