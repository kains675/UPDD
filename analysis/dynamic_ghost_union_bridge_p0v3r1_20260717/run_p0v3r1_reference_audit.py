#!/usr/bin/env python3
"""Build and audit frozen W4A union-solvated apex-bridge P0v3r1 cells."""
...
PREBUILD_INVENTORY = ANALYSIS_DIR / "P0V3R1_PREBUILD_INVENTORY.json"
SOURCE_ROOT = ANALYSIS_DIR / "u0_union_sources"
SOURCE_SUMMARY_JSON = ANALYSIS_DIR / "P0V3R1_SOURCE_SUMMARY.json"
SOURCE_SUMMARY_MD = ANALYSIS_DIR / "P0V3R1_SOURCE_SUMMARY.md"
KEEPER_PREFLIGHT = ANALYSIS_DIR / "P0V3R1_KEEPER_PREFLIGHT.json"
REFERENCE_ROOT = ANALYSIS_DIR / "p0v3r1_reference_audit"
REFERENCE_SUMMARY_JSON = ANALYSIS_DIR / "P0V3R1_REFERENCE_SUMMARY.json"
REFERENCE_SUMMARY_MD = ANALYSIS_DIR / "P0V3R1_REFERENCE_SUMMARY.md"
RUN_STATE = ANALYSIS_DIR / "P0V3R1_RUN_STATE.json"
...
def load_protocol_and_verify_freeze(
    ...
    if frozen.get("status") != "FROZEN_BEFORE_OFFICIAL_SOURCE_OR_P0V3R1_OUTPUT":
        raise ValueError("P0v3r1 frozen manifest status drifted")
    _hash_map(ANALYSIS_DIR, frozen.get("files") or {}, "P0v3r1 frozen file")
    ...
    if inherited.get("protocol_sha256") != sha256_file(BASE_PROTOCOL_PATH):
        raise ValueError("P0v3r1 inherited base protocol drifted")
    if protocol.get("scival_verdict") != "CONDITIONAL_APPROVE_P0V3R1_ONLY":
        raise ValueError("P0v3r1 SciVal verdict drifted")
    if (protocol.get("override") or {}).get("static_total_box_density_gate") != "REPORT_ONLY":
        raise ValueError("P0v3r1 density override drifted")
    if (protocol.get("override") or {}).get("equilibrium_density_claim_allowed") is not False:
        raise ValueError("P0v3r1 equilibrium-density prohibition drifted")
...
def _implementation_records() -> dict[str, dict[str, Any]]:
    return {
        ...
        "p0v3r1_runner": artifact_record(RUNNER_PATH),
    }
...
def build_prebuild_inventory() -> dict[str, Any]:
    ...
    frozen = json.loads(FROZEN_MANIFEST.read_text(encoding="utf-8"))
    payload = {
        "schema": "updd_dynamic_ghost_union_bridge_p0v3r1_prebuild_inventory_v1",
...
def load_prebuild_inventory(expected_digest: str | None = None) -> dict[str, Any]:
    ...
    if payload.get("inventory_digest") != inventory_digest(payload):
        raise ValueError("stored P0v3r1 prebuild inventory digest is invalid")
...
def verify_prebuild_inventory() -> dict[str, Any]:
    ...
    if frozen["inventory_digest"] != current["inventory_digest"]:
        raise ValueError(
            "P0v3r1 source/code drift after prebuild freeze: "
...
def run_source_worker(cell: UnionCell, inventory: dict[str, Any]) -> dict[str, Any]:
    ...
    _protocol, _base, _bridge_protocol, _ghost_protocol, frozen_cell = (
    ...
    try:
        ...
        audit["checks"]["caller_rng_state_unchanged"] = rng_after == rng_before
        if not audit["checks"]["caller_rng_state_unchanged"]:
        ...
        result = {
            "schema": "updd_dynamic_ghost_union_bridge_p0v3r1_source_cell_v1",
            "generated": time.strftime("%Y-%m-%d %H:%M:%S %z"),
            "status": audit["status"],
            "cell": {
                "cell_id": cell.cell_id,
...
def _write_source_summary(
    ...
    payload = {
        "schema": "updd_dynamic_ghost_union_bridge_p0v3r1_source_summary_v1",
        "generated": time.strftime("%Y-%m-%d %H:%M:%S %z"),
        "status": status,
        "inventory_digest": inventory["inventory_digest"],
        "context_count": 0,
    ...
    for cell in make_cells():
        ...
        if cell.source_dir.exists():
            _archive_partial(
                ...
                "incomplete or invalid P0v3r1 source build",
            )
        command = [
            sys.executable,
            str(RUNNER_PATH),
...
def load_source_summary_shallow(inventory: dict[str, Any]) -> dict[str, Any]:
    ...
    if payload.get("source_summary_digest") != source_summary_digest(payload):
        raise ValueError("stored P0v3r1 source summary digest is invalid")
    if payload.get("inventory_digest") != inventory["inventory_digest"]:
        raise ValueError("P0v3r1 source summary inventory drifted")
    if payload.get("status") != "PASS" or payload.get("n_completed") != 6:
        raise ValueError("P0v3r1 source summary is not a complete PASS")
    return payload
...
def run_keeper_preflight(inventory: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        "schema": "updd_dynamic_ghost_union_bridge_p0v3r1_keeper_v1",
        "generated": time.strftime("%Y-%m-%d %H:%M:%S %z"),
        "status": "PASS",
        "inventory_digest": inventory["inventory_digest"],
...
def validate_keeper_preflight(
    ...
    if payload.get("keeper_digest") != keeper_digest(payload):
        raise ValueError("P0v3r1 KEEPER digest is invalid")
    if payload.get("status") != "PASS" or payload.get("context_count") != 0:
        raise ValueError("P0v3r1 KEEPER preflight is not a context-free PASS")
    if payload.get("inventory_digest") != inventory["inventory_digest"]:
        raise ValueError("P0v3r1 KEEPER inventory digest drifted")
    if payload.get("source_summary_digest") != source_summary["source_summary_digest"]:
        raise ValueError("P0v3r1 KEEPER source summary digest drifted")
    if payload.get("n_cells") != len(make_cells()):
        raise ValueError("P0v3r1 KEEPER cell count drifted")
    return payload
...
def run_reference_worker(
    ...
    if not raw_guard["passed"]:
        result = {
            "schema": "updd_dynamic_ghost_union_bridge_p0v3r1_cell_v1",
            "generated": time.strftime("%Y-%m-%d %H:%M:%S %z"),
            "status": "REJECT_UNION_SOURCE",
            "cell": {
                "cell_id": cell.cell_id,
...
def validate_reference_result(
    ...
    if result["status"] == "P0V3_PASS":
        if not raw_passed or not all((result.get("bridge_checks") or {}).values()):
            raise ValueError(f"{cell.cell_id}: P0V3_PASS has a false gate")
...
def _write_reference_summary(
    ...
    median_elapsed = median(elapsed) if elapsed else None
    payload = {
        "schema": "updd_dynamic_ghost_union_bridge_p0v3r1_summary_v1",
        "generated": time.strftime("%Y-%m-%d %H:%M:%S %z"),
        "status": status,
        "platform": "Reference",
        "gpu_used": False,
    ...
    lines = [
        "# Union Apex Bridge P0v3r1 Reference Summary",
        "",
        f"Generated: {payload['generated']}",
        "",
        f"Status: **{status}**",
        "",
        "P0v3r1 used OpenMM Reference with zero MD/minimization steps and no GPU. "
    ...
    for cell in make_cells():
        ...
        if output_dir.exists():
            _archive_partial(
                ...
                "incomplete or invalid P0v3r1 Reference cell",
            )
        command = [
            sys.executable,
            str(RUNNER_PATH),
...
def _assert_atm_interpreter() -> None:
    if Path(sys.executable).resolve() != EXPECTED_ATM_PYTHON.resolve():
        raise RuntimeError(
            "P0v3r1 must run with the Track B ATM interpreter: "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.prepare_inventory:
            ...
            if PREBUILD_INVENTORY.exists():
                ...
                if existing["inventory_digest"] != payload["inventory_digest"]:
                    raise ValueError(
                        "refusing to overwrite a drifted P0v3r1 prebuild inventory"
                    )
            else:
        ...
        if args.summarize:
            ...
            print(json.dumps(summary, indent=2))
            return 0
        raise RuntimeError("unhandled P0v3r1 command")
