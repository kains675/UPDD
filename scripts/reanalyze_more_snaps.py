#!/usr/bin/env python3
"""Re-extract snapshots (n=25) and re-run MM-GBSA on existing UPDD calibration trajectories.

Rationale
---------
SciVal verdict (2026-04-22) on the 2QKI_Cp4 ladder diagnosed the sign-mismatch
vs sampling-inadequacy axes: observed z = 0.52 ⟹ the uncertainty band σ_wi
dominates the effect, i.e. sampling is inadequate rather than the magnitude
being wrong. Route A' of the v0.6.8 calibration plan therefore increases the
snapshot count per trajectory from n=10 (baseline) to n=25 per
Genheden, S. & Ryde, U. "Comparison of the Efficiency of the LIE and MM/GBSA
Methods to Calculate Ligand-Binding Energies" J. Comput. Chem. 2010, 31(4)
(DOI: 10.1002/jcc.21366), which identifies n≈25 as the knee of σ_wi decay for
peptide-class binders.

This Quick-Win script re-uses the existing DCD trajectories in
`outputs/*_calib_s*/mdresult/*_restrained.dcd` and writes results to parallel
directories `snapshots_n25/` and `mmgbsa_results_n25/` so that the baseline
n=10 artifacts remain untouched (R-7 archive discipline).

The script is a pure-stdlib subprocess orchestrator; it does not import UPDD
utils directly. Rev. 5 hardening: no GPU lock-in, idempotent, per-run logging.
"""
...
STATUS_OUT = os.path.join(PROJ, "outputs", "analysis", "reanalyze_more_snaps_status_20260422.json")
N_SNAPS = 25
...
def build_commands(run_dir: str, tag: str, platform: str) -> Tuple[List[str], List[str], Dict[str, str]]:
    ...
    snap_dir = os.path.join(run_dir, "snapshots_n25")
    mmgbsa_dir = os.path.join(run_dir, "mmgbsa_results_n25")
...
def main() -> int:
    ...
    for idx, (run_dir, tag) in enumerate(runs, 1):
        ...
        if os.path.isfile(summary_path):
            print(f"[{idx}/{len(runs)}] SKIP {tag} (mmgbsa_results_n25/mmgbsa_summary.json exists)")
        ...
        log_path = f"/tmp/reanalyze_more_snaps_{tag}.log"
        t0 = time.time()
        with open(log_path, "w") as log_fp:
            log_fp.write(f"# reanalyze_more_snaps tag={tag} platform={args.platform}\n")
