#!/usr/bin/env python
"""
utils/run_benchmark_1ebp.py
---------------------------
Benchmark runner: Compstatin 4W9A + C3c (PDB 1EBP) full pipeline validation.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
1. MD: 1EBP_clean.pdb (already has H, ions stripped in preprocess) →
...
def main():
    ...
    ap.add_argument("--outputdir", default=os.path.join(UPDD_ROOT, "outputs", "1EBP_benchmark"))
    ap.add_argument("--pdb", default=os.path.join(UPDD_ROOT, "target", "1EBP_clean.pdb"))
    args = ap.parse_args()
    ...
    staged_pdb = os.path.join(md_input_dir, "1EBP_compstatin_4W9A.pdb")
    ...
    print(f"  literature ΔG:   -6.40 kcal/mol (Wrighton 1996 Kd=20 μM)")
